...
•	Dicas:
o	Crie 6 useRef<TextInput>(null) diferentes (um para cada campo).
o	Use a propriedade maxLength={1} nos inputs.
o	No evento onChangeText, verifique se o texto não está vazio e use o .current?.focus() para avançar para o próximo campo.
...
•	Dicas:
o	Crie uma referência const clicadoRef = useRef(false);.
o	No clique do botão, verifique se clicadoRef.current é true. Se for, dê um return e aborte a função.
o	Se for false, mude para true, exiba a mensagem e use um setTimeout para mudar de volta para false após 3000ms.
...
•	Dicas:
o	Use const timerRef = useRef<ReturnType<typeof setInterval> | null>(null); para guardar o ID do timer.
o	Lembre-se de limpar o timer com clearInterval(timerRef.current) ao pausar, finalizar e quanto o componente for desmontado.
o	Nos botões, verifique se .current já está preenchido ao ser clicado. Isso impede múltiplos cliques desnecessários.
o	Lembre-se de converter os 25 minutos para 1500 segundos no seu useState inicial. 
o	A mudança de cor de fundo a cada 5 minutos equivale a cada 300 segundos (use o operador de resto da divisão % ou regras de três simples nos seus cálculos). Exemplo:
...
•	Dicas:
o	Conecte um useRef<FlatList>(null) na propriedade ref da sua lista.
o	Ao adicionar a mensagem, chame o método .current?.scrollToEnd({ animated: true }).
o	Envolva a rolagem em um setTimeout(..., 50) para dar tempo de o React desenhar o novo item antes da lista tentar rolar.
...
•	OBS:
o	Este exercício serve para fixar o conceito de que alterar o .current é uma operação silenciosa na memória do celular.
